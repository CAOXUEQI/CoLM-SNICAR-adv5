...
MODULE MOD_SnowSnicar
  ...
  ! !USES:
  USE MOD_Precision
  USE MOD_Vars_Global, only: maxsnl, nl_soil !+CAW
  USE MOD_SPMD_Task
  USE MOD_TimeManager !+CAW
  USE MOD_Namelist, only: use_snicar_lndice, use_snow_algae  ! +CAW ! +ALG
  use MOD_SnowAlgae, only: SnowAlgae_Growth_Model
...
! Temporay setting
  ...
  logical, parameter :: use_BC_snow_internal_mixing = .true.   !Apr. 4, 2024 
  character(len=256), parameter :: snicar_atm_type = 'sub-Arctic_summer'  ! Atmospheric profile used to obtain surface-incident spectral flux distribution
...
!-----------------------------------------------------------------------
  ...
  real(r8), allocatable :: sca_cff_vlm_airbbl(:,:)  !+CAW 
  real(r8), allocatable :: asm_prm_airbbl(:,:)      !+CAW 
...
!H. Wang
  ...
  subroutine SNICAR_AD_RT ( ipatch, flg_snw_ice, flg_slr_in, &
                            coszen, snl, h2osno, frac_sno, &
                            h2osno_liq, h2osno_ice, snw_rds, &
                            mss_cnc_aer_in, albsfc, albout, flx_abs, &
                            idate, patchtype, lnd_ice_flg, bbl_eff_rad, ice_density, lnd_ice_bc, bare_ice_albout, &
                            cellA, bioA, cellV, bioV, ocV, growth_period, deltim, t_soisno, pg_snow, solvd, solvi, solnd, solni, snowdp) !+CAW
     ...
     integer  , intent(in)  :: ipatch                        ! land patch index (col) [nbr]
     integer  , intent(in)  :: flg_snw_ice                  ! flag: =1 when called from CLM, =2 when called from CSIM
     integer  , intent(in)  :: flg_slr_in                   ! flag: =1 for direct-beam incident flux,=2 for diffuse incident flux
     real(r8) , intent(in)  :: coszen          ! cosine of solar zenith angle for next time step (col) [unitless]
     ...
     real(r8) , intent(in)  :: h2osno_liq     ( maxsnl+1:nl_soil )   ! liquid water content (col,lyr) [kg/m2]  +CAW
     real(r8) , intent(in)  :: h2osno_ice     ( maxsnl+1:nl_soil )   ! ice content (col,lyr) [kg/m2]  +CAW
     integer  , intent(in)  :: snw_rds        ( maxsnl+1:0 )   ! snow effective radius (col,lyr) [microns, m^-6]
     real(r8) , intent(in)  :: mss_cnc_aer_in ( maxsnl+1:0 , 1:sno_nbr_aer ) ! mass concentration of all aerosol species (col,lyr,aer) [kg/kg]
     real(r8) , intent(in)  :: albsfc         ( 1:numrad )     ! albedo of surface underlying snow (col,bnd) [frc]
     real(r8) , intent(out) :: albout         ( 1:numrad )     ! snow albedo, averaged into 2 bands (=0 if no sun or no snow) (col,bnd) [frc]
     ...
     integer  , intent(in)  :: idate(3)   ! +CAW
     integer  , intent(in)  :: patchtype  ! land patch type (0=soil, 1=urban or built-up, 2=wetland, 3=land ice, 4=deep lake) +CAW
     integer  , intent(out) :: lnd_ice_flg    ! +CAW
     real(r8) , intent(inout)  :: bbl_eff_rad(3)   ! effective airbubble radii within the ice  +CAW 
     real(r8) , intent(inout)  :: ice_density(3)   ! ice density  +CAW 
     real(r8) , intent(inout)  :: lnd_ice_bc(3)    ! equivalent BC  +CAW 
     real(r8) , intent(out) :: bare_ice_albout( 1:numrad )     ! ICE albedo, averaged into 2 bands (=0 if no sun or no snow) (col,bnd) [frc] +CAW 

     real(r8) , intent(out) :: cellA, bioA, cellV, bioV, ocV, growth_period
     real(r8) , intent(in)  :: deltim 
     real(r8) , intent(in)  :: snowdp 
     real(r8) , intent(in)  :: t_soisno(maxsnl+1:1)   
     real(r8) , intent(in)  :: pg_snow
     real(r8) , intent(in)  :: solvd, solvi, solnd, solni
     ...
     integer, parameter :: nlevice = nl_soil                 ! +CAW  nl_soil = 10


     ! Local variables representing single-column values of arrays:
     integer :: snl_lcl                                   ! negative number of snow layers [nbr]
     integer :: snw_rds_lcl(maxsnl+1:nlevice)                   ! snow effective radius [m^-6]
     real(r8):: flx_slrd_lcl(1:numrad_snw)                ! direct beam incident irradiance [W/m2] (set to 1)
     real(r8):: flx_slri_lcl(1:numrad_snw)                ! diffuse incident irradiance [W/m2] (set to 1)
     real(r8):: mss_cnc_aer_lcl(maxsnl+1:nlevice,1:sno_nbr_aer) ! aerosol mass concentration (lyr,aer_nbr) [kg/kg]
     real(r8):: h2osno_lcl                                ! total column snow mass [kg/m2]
     real(r8):: h2osno_liq_lcl(maxsnl+1:nlevice)                ! liquid water mass [kg/m2]
     real(r8):: h2osno_ice_lcl(maxsnl+1:nlevice)                ! ice mass [kg/m2]
     real(r8):: albsfc_lcl(1:numrad_snw)                  ! albedo of underlying surface [frc]
     real(r8):: ss_alb_snw_lcl(maxsnl+1:nlevice)                ! single-scatter albedo of ice grains (lyr) [frc]
     real(r8):: asm_prm_snw_lcl(maxsnl+1:nlevice)               ! asymmetry parameter of ice grains (lyr) [frc]
     real(r8):: ext_cff_mss_snw_lcl(maxsnl+1:nlevice)           ! mass extinction coefficient of ice grains (lyr) [m2/kg]
     real(r8):: ss_alb_aer_lcl(sno_nbr_aer)               ! single-scatter albedo of aerosol species (aer_nbr) [frc]
     real(r8):: asm_prm_aer_lcl(sno_nbr_aer)              ! asymmetry parameter of aerosol species (aer_nbr) [frc]
     real(r8):: ext_cff_mss_aer_lcl(sno_nbr_aer)          ! mass extinction coefficient of aerosol species (aer_nbr) [m2/kg]

     real(r8):: sca_cff_vlm_airbbl_lcl(maxsnl+1:nlevice)! single-scatter albedo of air bbls (lyr) [frc] ! +CAW
     real(r8):: abs_cff_mss_ice_lcl(maxsnl+1:nlevice)   ! +CAW
     real(r8):: vlm_frac_air(maxsnl+1:nlevice)          ! +CAW
     ...
     real(r8):: h2osoi_ice_lcl(maxsnl+1:nlevice)       ! liquid water mass [kg/m2] +CAW
     real(r8):: h2osoi_liq_lcl(maxsnl+1:nlevice)       ! liquid water mass [kg/m2]  +CAW 
     real(r8):: temp2                                     
     real(r8):: temp1
     real(r8):: nreal   
     ...
     ! ----------------Snow Algae--------------------    ! +ALG
     ! The base case dry cell pigment mass fractions are:                                                    
     ! 1.5%  chlorophyll-a, 0.5% chlorophyll-b, 5.0% photoprotective carotenoids, 0 photosynthetic carotenoids                     
     ! This is similar to the “medium” scenario from Cook et al. (2017) but with a chlorophyll b/chlorophyll a ratio                    
     ! (0.33) in line with measurements of C. nivalis from Remias et al. (2005).
     real(r8):: ss_alb_alg_lcl       ! single-scatter albedo of snow algae                 ! +ALG
     real(r8):: ext_cff_mss_alg_lcl  ! mass extinction coefficient of snow algae           ! +ALG
     real(r8):: asm_prm_alg_lcl      ! asymmetry parameter of snow algae                   ! +ALG
     real(r8):: ss_alb_alg(1:numrad_snw)         ! +ALG
     real(r8):: ext_cff_mss_alg(1:numrad_snw)    ! +ALG
     real(r8):: asm_prm_alg(1:numrad_snw)        ! +ALG
     real(r8):: cell_nbr_conc   ! snow algae concentration [cells/mL]
     real(r8):: mean_vol_cell   ! mean algal cell volume (3rd moment of Gaussian distribution) by layer  [um^3/cell]
     real(r8):: mass_per_cell   ! mean mass per cell by layer  [kg/cell] 
     real(r8):: mss_cnc_alg(maxsnl+1:nlevice)   ! mass concentration of algae by layer  [kg/kg]
     real(r8):: L_alg(maxsnl+1:nlevice)         ! algae mass in snow layer (lyr,nbr_aer) [kg/m2]
     real(r8):: tau_alg(maxsnl+1:nlevice)       ! algae optical depth (lyr,nbr_aer) [unitless]
     real(r8), parameter :: alg_rds = 10        ! mean cell radius (um)
     real(r8), parameter :: sigma_alg = alg_rds * 0.1_r8  ! standard deviation of Gaussian size distribution [um] (consistent with Mie)
     real(r8), parameter :: rho_alg = 1080.0_r8   ! algae density [kg/m3] (consistent with Mie calculations)
     
     data ss_alb_alg(1:5)       / 0.6788500755_r8,  0.9978473154_r8, &
             0.9996598743_r8,  0.9970978945_r8, 0.9659971285_r8/
     data ext_cff_mss_alg(1:5)  / 140.5924635552_r8,  142.6430608253_r8, &
             144.1341009471_r8,  145.2219258103_r8, 148.9769774589_r8/
     data asm_prm_alg(1:5)      / 0.9276138184_r8,  0.8488713639_r8, &
             0.8437214041_r8,  0.8409551319_r8, 0.8381167973_r8/    


#ifdef MODAL_AER
     !mgf++
     real(r8) :: rds_bcint_lcl(maxsnl+1:nlevice)       ! effective radius of within-ice BC [nm]
     real(r8) :: rds_bcext_lcl(maxsnl+1:nlevice)       ! effective radius of external BC [nm]
     !mgf--
#endif
     ...
     real(r8):: albedo                              ! temporary snow albedo [frc]
     real(r8):: flx_sum                             ! temporary summation variable for NIR weighting
     real(r8):: flx_sum2(maxsnl+1:nlevice+1)     ! temporary summation variable for NIR weighting !CAW 
     real(r8):: albout_lcl(numrad_snw)              ! snow albedo by band [frc]
     real(r8):: flx_abs_lcl(maxsnl+1:nlevice+1,numrad_snw)  ! absorbed flux per unit incident flux at top of snowpack (lyr,bnd) [frc]

     real(r8):: L_snw(maxsnl+1:nlevice)                   ! h2o mass (liquid+solid) in snow layer (lyr) [kg/m2]
     real(r8):: tau_snw(maxsnl+1:nlevice)                 ! snow optical depth (lyr) [unitless]
     real(r8):: L_aer(maxsnl+1:nlevice,sno_nbr_aer)       ! aerosol mass in snow layer (lyr,nbr_aer) [kg/m2]
     real(r8):: tau_aer(maxsnl+1:nlevice,sno_nbr_aer)     ! aerosol optical depth (lyr,nbr_aer) [unitless]
     real(r8):: tau_sum                             ! cumulative (snow+aerosol) optical depth [unitless]
     real(r8):: tau_elm(maxsnl+1:nlevice)                 ! column optical depth from layer bottom to snowpack top (lyr) [unitless]
     real(r8):: omega_sum                           ! temporary summation of single-scatter albedo of all aerosols [frc]
     real(r8):: g_sum                               ! temporary summation of asymmetry parameter of all aerosols [frc]

     real(r8):: tau(maxsnl+1:nlevice)                     ! weighted optical depth of snow+aerosol layer (lyr) [unitless]
     real(r8):: omega(maxsnl+1:nlevice)                   ! weighted single-scatter albedo of snow+aerosol layer (lyr) [frc]
     real(r8):: g(maxsnl+1:nlevice)                       ! weighted asymmetry parameter of snow+aerosol layer (lyr) [frc]
     real(r8):: tau_star(maxsnl+1:nlevice)                ! transformed (i.e. Delta-Eddington) optical depth of snow+aerosol layer
                                                    ! (lyr) [unitless]
     real(r8):: omega_star(maxsnl+1:nlevice)              ! transformed (i.e. Delta-Eddington) SSA of snow+aerosol layer (lyr) [frc]
     real(r8):: g_star(maxsnl+1:nlevice)                  ! transformed (i.e. Delta-Eddington) asymmetry paramater of snow+aerosol layer
                                                    ! (lyr) [frc]
     ...
     integer :: lyr_typ(maxsnl+1:nlevice)        ! 1= snow layer, 2 = ice layer +CAW
     integer :: lnd_ice

     real(r8):: F_abs(maxsnl+1:nlevice)                   ! net absorbed radiative energy (lyr) [W/m^2]
     real(r8):: F_abs_sum                           ! total absorbed energy in column [W/m^2]
     real(r8):: F_sfc_pls                           ! upward radiative flux at snowpack top [W/m^2]
     real(r8):: F_btm_net                           ! net flux at bottom of snowpack [W/m^2]
     real(r8):: energy_sum                          ! sum of all energy terms; should be 0.0 [W/m^2]
     ...
     integer :: yr, mon, day, mday, aindex(2), caldaym(13)
     ...
     real(r8):: &
        trndir(maxsnl+1:nlevice+1)  , & ! solar beam down transmission from top
        trntdr(maxsnl+1:nlevice+1)  , & ! total transmission to direct beam for layers above
        trndif(maxsnl+1:nlevice+1)  , & ! diffuse transmission to diffuse beam for layers above
        rupdir(maxsnl+1:nlevice+1)  , & ! reflectivity to direct radiation for layers below
        rupdif(maxsnl+1:nlevice+1)  , & ! reflectivity to diffuse radiation for layers below
        rdndif(maxsnl+1:nlevice+1)  , & ! reflectivity to diffuse radiation for layers above
        dfdir(maxsnl+1:nlevice+1)   , & ! down-up flux at interface due to direct beam at top surface
        dfdif(maxsnl+1:nlevice+1)   , & ! down-up flux at interface due to diffuse beam at top surface
        dftmp(maxsnl+1:nlevice+1)       ! temporary variable for down-up flux at interface

     real(r8):: &
        rdir(maxsnl+1:nlevice)    , & ! layer reflectivity to direct radiation
        rdif_a(maxsnl+1:nlevice)  , & ! layer reflectivity to diffuse radiation from above
        rdif_b(maxsnl+1:nlevice)  , & ! layer reflectivity to diffuse radiation from below
        tdir(maxsnl+1:nlevice)    , & ! layer transmission to direct radiation (solar beam + diffuse)
        tdif_a(maxsnl+1:nlevice)  , & ! layer transmission to diffuse radiation from above
        tdif_b(maxsnl+1:nlevice)  , & ! layer transmission to diffuse radiation from below
        trnlay(maxsnl+1:nlevice)      ! solar beam transm for layer (direct beam only)
     ...
     !-----------------------------------------------------------------------
      ...
      ! Constants for non-spherical ice particles
      real(r8) :: g_b2(7)
      real(r8) :: g_b1(7)
      real(r8) :: g_b0(7)
      real(r8) :: g_F07_c2(7)
      ...
      integer, parameter :: num_wvl_ct = 5                   
      real(r8), parameter :: wvl_ct(num_wvl_ct)  = (/ 0.5_r8, 0.85_r8, 1.1_r8, 1.35_r8, 3.25_r8 /)  ! band center wavelength (um) for 5-band case
  ...
  !       )
     ...
     dif_b(1:5)     &
      ...
      if (use_snicar_lndice) then 
               ...
               call julian2monthday(yr, day, mon, mday)
               calday_curr = idate(2)
               aindex(1) = 2
       ...
       ! +CAW - start
              if ( (patchtype == 3)  .and. (use_snicar_lndice) .and. (mon==6 .or. mon==7 .or. mon==8) &  
              .and. (bbl_eff_rad(2) .gt. 0) .and. (ice_density(2) .gt. 0 ) .and. (lnd_ice_bc(2) .gt. 0) ) then  ! land ice

               if (bbl_eff_rad(1) .lt. 0) bbl_eff_rad(1) = bbl_eff_rad(2)
               if (bbl_eff_rad(3) .lt. 0) bbl_eff_rad(3) = bbl_eff_rad(2)
               if (ice_density(1) .lt. 0) ice_density(1) = ice_density(2)
               if (ice_density(3) .lt. 0) ice_density(3) = ice_density(2)
               if (lnd_ice_bc(1)  .lt. 0) lnd_ice_bc(1)  = lnd_ice_bc(2)
               if (lnd_ice_bc(3)  .lt. 0) lnd_ice_bc(3)  = lnd_ice_bc(2)

               !write(*,*) "MOD_SnowSnicar ice_density ----->", ice_density
                 ...
                 !bare_ice_indx_lcl = bare_ice_indx(g_idx)
                 !bare_ice_indy_lcl = bare_ice_indy(g_idx)
                 ...
                 !endif
                 ice_bc_conc_wgted = lnd_ice_bc(aindex(1))*wt1 + lnd_ice_bc(aindex(2))*wt2 
                 ice_bc_conc_wgted=ice_bc_conc_wgted*1.e-09_r8 ! convert from ng/g -> kg/kg 
               
                 ice_density_wgted = ice_density(aindex(1))*wt1 + ice_density(aindex(2))*wt2
                 bbl_eff_rad_wgted = bbl_eff_rad(aindex(1))*wt1 + bbl_eff_rad(aindex(2))*wt2
          ...
          ! Zero absorbed radiative fluxes:
          do i=maxsnl+1,snl_btm+1,1 !+CAW - extend total # of layers
             flx_abs_lcl(i,:)   = 0._r8
             flx_sum2(i)        = 0._r8
          enddo
          
          ! CAW - zero mass aerosol concentration over snow & ice layers 
          do i=maxsnl+1,snl_btm,1
          ...
          !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
          if ( (coszen > 0._r8) .and. (lnd_ice ==1) ) then
                  snl_lcl           =  0  
                ...
                h2osoi_liq_lcl(:) = h2osno_liq(:) ! +CAW
                h2osoi_ice_lcl(:) = h2osno_ice(:) ! +CAW
                
                h2osno_liq_lcl(1:snl_btm) = h2osno_liq(1:snl_btm) ! +CAW
                h2osno_ice_lcl(1:snl_btm) = h2osno_ice(1:snl_btm) ! +CAW
                   ...
                   if (flg_slr_in == 1) then
                      do i=snl_top,snl_btm,1
                         if (i<kfrsnl) then ! +CAW
                            ...
                            !write(*,*) "MOD_SnowSnicar ice_density_wgted ----->", ice_density        
                   ...
                   do i=snl_top,snl_btm,1
                    ...
                    !mgf--
                     ...
                     ! endif
                      ...
                      ! --------------------------------------Snow algae--------------------------------------------
                      ! Assuming that each snow layer contains snow algae and the algal density is the same.  ! +ALG
                      ss_alb_alg_lcl      = ss_alb_alg(bnd_idx)
                      ext_cff_mss_alg_lcl  = ext_cff_mss_alg(bnd_idx)
                      asm_prm_alg_lcl    = asm_prm_alg(bnd_idx)

                      ! Snow algae are restricted to the top snow layer, and algae in the remaining snow layers are removed.  ! +ALG
                      ! burden and optical thickness of algae  
                      if (use_snow_algae) then  

                        call SnowAlgae_Growth_Model(ipatch, snowdp, t_soisno, snl, &
                                 pg_snow, solvd, solvi, solnd, solni, deltim, &
                                 cellA, bioA, cellV, bioV, ocV, growth_period)     

                        if (i == snl_top) then
                           cell_nbr_conc = cellV * 0.001_r8  ! calculate at top layer; cell/L -> cell/ml
    
                           if (cell_nbr_conc > 0.0_r8) then  
                              ! mean algal cell volume (3rd moment of Gaussian distribution) by layer:
                              mean_vol_cell = 4.0_r8/3.0_r8 * pi * (alg_rds**3 + 3.0_r8 * alg_rds * sigma_alg**2)  ! [um^3/cell]
                              ! mean mass per cell by layer:
                              mass_per_cell = mean_vol_cell * 1.0e-18_r8 * rho_alg  ! [kg/cell]
                              ! mass concentration of algae by layer
                              mss_cnc_alg(i) = cell_nbr_conc * 1000.0_r8 * mass_per_cell  ! [kg/kg]

                              L_alg(i) = L_snw(i) * mss_cnc_alg(i)
                              tau_alg(i) = L_alg(i) * ext_cff_mss_alg_lcl

                              tau_sum   = tau_sum + tau_alg(i)
                              omega_sum = omega_sum + (tau_alg(i) * ss_alb_alg_lcl)
                              g_sum     = g_sum + (tau_alg(i) * ss_alb_alg_lcl * asm_prm_alg_lcl)
                           else
                              L_alg(i) = 0.0_r8
                              tau_alg(i) = 0.0_r8
                           end if
                        else
                         ! Snow algae in non-top snow layers are removed.
                            L_alg(i) = 0.0_r8
                            tau_alg(i) = 0.0_r8
                        end if
                      end if
...
!                endif
             ...
             ! Weight output NIR albedo appropriately
             bare_ice_albout(1) = albout_lcl(1)
             flx_sum         = 0._r8
             do bnd_idx= nir_bnd_bgn,nir_bnd_end
                flx_sum = flx_sum + flx_wgt(bnd_idx)*albout_lcl(bnd_idx)
             enddo
             bare_ice_albout(2) = flx_sum / sum(flx_wgt(nir_bnd_bgn:nir_bnd_end))
       ...
       lnd_ice_flg = lnd_ice
       !write(*,*) "MOD_SnowSnicar lnd_ice_flg ----->", lnd_ice_flg 
       !write(*,*) "MOD_SnowSnicar bare_ice_albout ----->", bare_ice_albout
       !write(*,*) "MOD_SnowSnicar albsfc ----->", albsfc
          ...
          ! Set variables specific to ELM
            if (flg_snw_ice == 1) then
               ...
               if (snl > -1) then
                  ...
                  snw_rds_lcl(maxsnl+1:0)    =  snw_rds(:)
               endif
     ...
     !H. Wang
               ...
               do i=snl_top,snl_btm,1
                  ...
                  ! Start BC/dust-snow internal mixing for wavelength<=1.2um
                       if (wvl_doint <= 1.2_r8) then
                          ...
                          if ( use_BC_snow_internal_mixing .and. (mss_cnc_aer_lcl(i,1) > 0._r8) ) then
                          ...
                          if ( use_dust_snow_internal_mixing .and. (tot_dst_snw_conc > 0._r8) ) then
                             do idb=1, size_bins
            ...
            ! Weight output NIR absorbed layer fluxes (flx_abs) appropriately
            flx_abs(:,1) = flx_abs_lcl(maxsnl+1:1,1)
            do i=snl_top,1,1
               flx_sum = 0._r8
               do bnd_idx= nir_bnd_bgn,nir_bnd_end
                  flx_sum = flx_sum + flx_wgt(bnd_idx)*flx_abs_lcl(i,bnd_idx)
 ...
 !       )
  ...
  subroutine SnowOptics_init( fsnowoptics, ficephyprop )

      USE MOD_NetCDFSerial

      IMPLICIT NONE

      character(len=256), intent(in) :: fsnowoptics, ficephyprop     ! snow optical properties file name
  ...
  !-----------------------------------------------------------------------
      ...
      IF (p_is_master) THEN
         ...
         write(iulog,*) subname,trim(ficephyprop)          ! +CAW
      ENDIF
      ...
      CALL ncio_read_bcast_serial (ficephyprop, 'sca_cff_vlm_airbbl', sca_cff_vlm_airbbl)  ! +CAW 
      CALL ncio_read_bcast_serial (ficephyprop, 'asm_prm_airbbl', asm_prm_airbbl)          ! +CAW
...
!   subroutine BareIceGrd(num_nourbanc, filter_nourbanc)
!    !------------------------------------------------------------------
!    ! bare ice grid information
!    !------------------------------------------------------------------
!    !   use spmdMod    , only : p_is_master
...
!       if (use_snicar_lndice) then
!               if(p_is_master)  write(iulog,*) 'Attempting to re grid bare ice data CAW .....'
