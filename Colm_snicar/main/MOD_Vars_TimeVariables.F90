...
! Time-varying state variables which reaquired by restart run
     ...
     real(r8), allocatable :: up_dir_per   (:)
     real(r8), allocatable :: up_dif_per   (:)
     real(r8), allocatable :: phi_srf_dif   (:)
     real(r8), allocatable :: phi_srf_dir   (:)
     real(r8), allocatable :: phi_dir_per   (:,:)
     real(r8), allocatable :: phi_dif_per   (:,:)
...
! ------------------------------------------------------
     ...
     IF (p_is_worker) THEN
        ...
        IF (numpatch > 0) THEN
           ...
           allocate (phi_dir_per              (maxsnl+1:nl_lake,numpatch)); phi_dir_per      (:,:) = spval
           allocate (phi_dif_per              (maxsnl+1:nl_lake,numpatch)); phi_dif_per      (:,:) = spval
           allocate (up_dir_per                      (numpatch)); up_dir_per        (:) = spval
           allocate (up_dif_per                      (numpatch)); up_dif_per        (:) = spval
           allocate (phi_srf_dif                      (numpatch)); phi_srf_dif        (:) = spval
           allocate (phi_srf_dir                      (numpatch)); phi_srf_dir        (:) = spval
  ...
  SUBROUTINE deallocate_TimeVariables ()
     ...
     IF (p_is_worker) THEN
        ...
        IF (numpatch > 0) THEN
           ...
           deallocate (up_dir_per                 )
           deallocate (up_dif_per                 )
           deallocate (phi_srf_dif                 )
           deallocate (phi_srf_dir                 )
           deallocate (phi_dir_per                 )
           deallocate (phi_dif_per                 )
           deallocate (z_sno                  )
           deallocate (dz_sno                 )
           deallocate (t_soisno               )
...
!End Ozone stress variables
  ...
  SUBROUTINE WRITE_TimeVariables (idate, lc_year, site, dir_restart)
     ...
     CALL ncio_define_dimension_vector (file_restart, landpatch, 'snowlake',     nl_lake-maxsnl)
...
ENDIF
     ...
     CALL ncio_write_vector (file_restart, 'phi_dir_per  '   , 'snowlake', nl_lake-maxsnl, 'patch', landpatch, phi_dir_per      , compress)
     CALL ncio_write_vector (file_restart, 'phi_dif_per  '   , 'snowlake', nl_lake-maxsnl, 'patch', landpatch, phi_dif_per      , compress)
     CALL ncio_write_vector (file_restart, 'up_dir_per',    'patch', landpatch, up_dir_per, compress) 
     CALL ncio_write_vector (file_restart, 'up_dif_per',    'patch', landpatch, up_dif_per, compress)  
     CALL ncio_write_vector (file_restart, 'phi_srf_dif',    'patch', landpatch, phi_srf_dif, compress)
     CALL ncio_write_vector (file_restart, 'phi_srf_dir',    'patch', landpatch, phi_srf_dir, compress)
...
ENDIF
  ...
  SUBROUTINE READ_TimeVariables (idate, lc_year, site, dir_restart)
     ...
     CALL ncio_read_vector (file_restart, 'up_dir_per  ',    landpatch, up_dir_per     )
     CALL ncio_read_vector (file_restart, 'up_dif_per  ',    landpatch, up_dif_per     )
     CALL ncio_read_vector (file_restart, 'phi_srf_dif  ',    landpatch, phi_srf_dif     )
     CALL ncio_read_vector (file_restart, 'phi_srf_dir  ',    landpatch, phi_srf_dir     )
     CALL ncio_read_vector (file_restart, 'phi_dir_per  '   , nl_lake-maxsnl, landpatch, phi_dir_per      )
     CALL ncio_read_vector (file_restart, 'phi_dif_per  '   , nl_lake-maxsnl, landpatch, phi_dif_per      )
...
ENDIF
  ...
  SUBROUTINE check_TimeVariables ()
     ...
     CALL check_vector_data ('phi dir per     []     ', phi_dir_per     )
     CALL check_vector_data ('phi dif per     []     ', phi_dif_per     )
     CALL check_vector_data ('up dir per     []     ', up_dir_per     )
     CALL check_vector_data ('up dif per     []     ', up_dif_per     )
     CALL check_vector_data ('phi_srf_dif     []     ', phi_srf_dif     )
     CALL check_vector_data ('phi_srf_dir     []     ', phi_srf_dir     )
