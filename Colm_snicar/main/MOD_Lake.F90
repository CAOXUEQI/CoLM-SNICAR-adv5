...
! ---------------- local variables in lake/snow/soil temperature calculation --------------
  ...
  real(r8) lake_debug_surface_shortwave
  real(r8) lake_debug_lw_in
  real(r8) lake_debug_lw_out
  real(r8) lake_debug_sensible
  real(r8) lake_debug_latent
  real(r8) lake_debug_top_diff
  real(r8) lake_debug_bot_diff
  real(r8) lake_debug_net_diff
  real(r8) lake_debug_phase_ice_before(1:nl_lake)
  real(r8) lake_debug_mix_t_before(1:nl_lake)
  real(r8) lake_debug_mix_ice_before(1:nl_lake)
  real(r8) lake_debug_mix_rhow_before(1:nl_lake)
  logical  lake_debug_mixed
...
! ======================================================================
   ...
   !The actual heat flux from the ground interface into the lake, not including the light that penetrates the surface.
         ...
         lake_debug_surface_shortwave = betaprime*sabg
         lake_debug_lw_in = forc_frl
         lake_debug_lw_out = olrg
         lake_debug_sensible = fseng
         lake_debug_latent = htvp*fevpg
         write(*,*) 'LAKETEMP_DEBUG surface_budget', 'scheme', 'original', &
                    'sw_surface', lake_debug_surface_shortwave, 'lw_in', lake_debug_lw_in, &
                    'lw_out', lake_debug_lw_out, 'sensible', lake_debug_sensible, &
                    'latent', lake_debug_latent, 'fgrnd1', fgrnd1, &
                    't_grnd', t_grnd, 't_lake1', t_lake(1), 'snl', snl
   ...
   !------------------------------------------------------------
         ...
         do j = 1, nl_lake
            write(*,*) 'LAKETEMP_DEBUG lake_property_layer', 'j', j, &
                       't_lake', t_lake(j), 'icefrac', lake_icefrac(j), &
                       'rhow', rhow(j), 'cv_lake', cv_lake(j), &
                       'tk_lake', tk_lake(j), 'dz_lake', dz_lake(j), 'z_lake', z_lake(j)
         enddo
   ...
   ! to prevent abrupt change in heat content due to changing heat capacity with phase change.
      ...
      IF (.not. DEF_USE_SNICAR .or. present(urban_call)) THEN
         lakealb_direct_shortwave = -9999
         lakealb_diffuse_shortwave= -9999
         lakealb_direct_vis = -9999
         lakealb_diffuse_vis= -9999
         lakealb_direct_nir = -9999
         lakealb_diffuse_nir= -9999 
         phi_dir_per(:) = 0.
         phi_dif_per(:) = 0.
         up_dir_per = 0.
         up_dif_per = 0.   
         ...
         IF(DEF_USE_ORIGINAL)THEN
            ...
            if ((t_grnd > tfrz .and. t_lake(1) > tfrz .and. snl == 0)) then      !no snow cover, unfrozen layer lakes
               ...
               phi_srf_dif = sabgvd+sabgnd
               phi_srf_dir = sabgv+sabgn
               phi(:) = 0.
               phi_soil = 0.
            end if
            ...
            write(*,*)'forcing all albedo',forc_sols,forc_solsd,forc_soll,forc_solld
            write(*,*)'in all energy',sabgv,sabgvd,sabgn,sabgnd
            write(*,*)'up all energy',forc_sols-sabgv,forc_solsd-sabgvd,forc_soll-sabgn,forc_solld-sabgnd
            ...
            if(forc_sols/=0) then 
               write(*,*)'energy_albedo',(forc_sols-sabgv)/forc_sols
            endif 
         ! ENDIF
                  ...
                  if (flg_slr_in == 1) then
                     ...
                     do i=snl_top,nbr_lyr,1
                        if (i <=0 ) then     !snow
                           ...
                           ! if(bnd_idx == 1) then 
                           !    ext_cff_mss_snw_lcl(i) = 2.5/ice_density_wgted
                           !    sca_cff_vlm_airbbl_lcl(i) = (ext_cff_mss_snw_lcl(i)-abs_cff_mss_ice_lcl(i))&
                           !                                     *ice_density_wgted/vlm_frac_air(i)
                           !    write(*,*)'adjust sca_cff_vlm_airbbl_lcl for band 1',sca_cff_vlm_airbbl_lcl(i),ext_cff_mss_snw_lcl(i)
                           ! endif
                           ...
                           if(bnd_idx == 1) then 
                              ext_cff_mss_wtr_avg_clr(bnd_idx,:) = 0.239
                  ...
                  elseif (flg_slr_in == 2) then
                     ...
                     do i=snl_top,nbr_lyr,1
                        if (i <= 0) then
                           ...
                           if(bnd_idx == 1) then 
                              ext_cff_mss_wtr_avg_cld(bnd_idx) = 0.239
...
! matrix and solve system
      ...
      ENDIF
            do j = 1, nl_lake
         lake_debug_top_diff = 0._r8
         lake_debug_bot_diff = 0._r8
         lake_debug_net_diff = 0._r8
         if (j > lb) lake_debug_top_diff = fnx(j-1)
         if (j < nl_lake+nl_soil) lake_debug_bot_diff = fnx(j)
         if (j == lb) then
            lake_debug_net_diff = cnfac*lake_debug_bot_diff + fgrnd1
         else if (j < nl_lake+nl_soil) then
            lake_debug_net_diff = cnfac*(lake_debug_bot_diff - lake_debug_top_diff)
         else
            lake_debug_net_diff = -cnfac*lake_debug_top_diff
         endif
         write(*,*) 'LAKETEMP_DEBUG tridia_layer_before', 'j', j, &
                    'tx_bef', tx_bef(j), 'phix', phix(j), &
                    'fgrnd1_if_top', merge(fgrnd1,0._r8,j==lb), &
                    'fnx_top', lake_debug_top_diff, 'fnx_bottom', lake_debug_bot_diff, &
                    'net_diffusion_term', lake_debug_net_diff, 'cvx', cvx(j), &
                    'factx', factx(j), 'tkix_below', tkix(j), 'rhs_r', r(j)
      enddo
...
!------------------------------------------------------------
         ...
         call tridia (nl_sls, a(lb:), b(lb:), c(lb:), r(lb:), tx(lb:))
               ...
               do j = 1, nl_lake
         write(*,*) 'LAKETEMP_DEBUG tridia_layer_after', 'j', j, &
                    'tx_bef', tx_bef(j), 'tx_after', tx(j), &
                    'delta_t', tx(j)-tx_bef(j), 'rhs_r', r(j), &
                    'a', a(j), 'b', b(j), 'c', c(j)
      enddo
...
!------------------------------------------------------------
      ...
      imelt_lake(:) = 0
      t_lake_bef(:) = t_lake(:)
      lake_debug_phase_ice_before(:) = lake_icefrac(:)
      write(*,*) 'LAKETEMP_DEBUG phase_begin', 't_grnd', t_grnd, &
                 'top_lake_t', t_lake(1), 'top_icefrac', lake_icefrac(1), &
                 'scv', scv, 'snowdp', snowdp
      ...
      do j = 1,nl_lake
         ...
         if (imelt_lake(j) > 0) then
            ...
            xmf = xmf + melt*hfus
            write(*,*) 'LAKETEMP_DEBUG lake_phase_change', 'j', j, &
            'imelt', imelt_lake(j), 't_before', t_lake_bef(j), &
            't_after', t_lake(j), 'ice_before', lake_debug_phase_ice_before(j), &
            'ice_after', lake_icefrac(j), 'heatavail', heatavail, &
            'melt', melt, 'heatrem', heatrem, 'xmf', xmf
end if
      end do
      ...
      write(*,*) 'LAKETEMP_DEBUG snow_lake_phase_summary', &
                 'top_t_before_phase', t_lake_bef(1), 'top_t_after', t_lake(1), &
                 'top_ice_before', lake_debug_phase_ice_before(1), &
                 'top_ice_after', lake_icefrac(1), 'wice_snow0', wice_soisno(0), &
                 'wliq_snow0', wliq_soisno(0), 'xmf', xmf, 'sm', sm
      do j = 1, nl_lake
         write(*,*) 'LAKETEMP_DEBUG phase_layer_after', 'j', j, &
                    't_before_phase', t_lake_bef(j), 't_after_phase', t_lake(j), &
                    'delta_t_phase', t_lake(j)-t_lake_bef(j), &
                    'ice_before_phase', lake_debug_phase_ice_before(j), &
                    'ice_after_phase', lake_icefrac(j)
      enddo
      ! write(*,*)'after xmf', xmf
...
! write(*,*)'before Convective mixing'
      lake_debug_mix_t_before(:) = t_lake(:)
      lake_debug_mix_ice_before(:) = lake_icefrac(:)
      lake_debug_mixed = .false.
do j = 1, nl_lake 
...
enddo
      ...
      do j = 1, nl_lake
         rhow(j) = (1.-lake_icefrac(j))*1000.*(1.0-1.9549e-05*(abs(t_lake(j)-277.))**1.68) &
                     + lake_icefrac(j)*denice
         lake_debug_mix_rhow_before(j) = rhow(j)
      end do
      ...
      do j = 1, nl_lake-1
         ...
         if (rhow(j)>rhow(j+1)+ 1.e-6 .or. (lake_icefrac(j)<1.0 .and. lake_icefrac(j+1)>0.)) then
                        write(*,*) 'LAKETEMP_DEBUG convective_mix_trigger', 'j', j, &
                       'rhow_upper', rhow(j), 'rhow_lower', rhow(j+1), &
                       'ice_upper', lake_icefrac(j), 'ice_lower', lake_icefrac(j+1), &
                       't_upper', t_lake(j), 't_lower', t_lake(j+1)
            do i = 1, j+1
            ...
            write(*,*) 'LAKETEMP_DEBUG convective_mix_average', 'j', j, &
                       'qav', qav, 'iceav', iceav, 'nav', nav, &
                       'tav_froz_C', tav_froz, 'tav_unfr_C', tav_unfr
         end if

         if (nav > 0.) then
            lake_debug_mixed = .true.
            do i = 1, j+1
      ...
      do j = 1, nl_lake
         write(*,*) 'LAKETEMP_DEBUG convective_mix_layer_after', 'j', j, &
                    'mixed_any', lake_debug_mixed, &
                    't_before_mix', lake_debug_mix_t_before(j), 't_after_mix', t_lake(j), &
                    'delta_t_mix', t_lake(j)-lake_debug_mix_t_before(j), &
                    'ice_before_mix', lake_debug_mix_ice_before(j), &
                    'ice_after_mix', lake_icefrac(j), &
                    'rhow_before_mix', lake_debug_mix_rhow_before(j), 'rhow_after_mix', rhow(j)
      enddo
