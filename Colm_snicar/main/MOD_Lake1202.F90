...
! END SNICAR model variables
           ...
           phi_dir_per,phi_dif_per,up_dir_per,up_dif_per,phi_srf,urban_call   )
   ...
   ! to prevent abrupt change in heat content due to changing heat capacity with phase change.
      ...
      IF (.not. DEF_USE_SNICAR .or. present(urban_call)) THEN
         ...
         !-------------------------------find kfrsnl--------------------------------
            ...
            snw_rds_lcl(1:nbr_lyr)    = 1000
            ice_density_wgted = 916.99999
                  ...
                  if (flg_slr_in == 1) then
                     ...
                     do i=snl_top,nbr_lyr,1
                        ...
                        if (i <=0 ) then     !snow
                           ...
                           write(*,*)i,'ext_cff_mss_snw_lcl',ext_cff_mss_snw_lcl(i)
                           ...
                           if(bnd_idx == 1) then 
                              ext_cff_mss_wtr_avg_clr(bnd_idx,:) = 0.963
                              ss_alb_wtr_avg_clr(bnd_idx,slr_zen+1) = 0.001/0.963
                           endif 
                  ...
                  elseif (flg_slr_in == 2) then
                     ...
                     do i=snl_top,nbr_lyr,1
                        if (i <= 0) then
                           ...
                           if(bnd_idx == 1) then 
                              ext_cff_mss_wtr_avg_cld(bnd_idx) = 0.963
                              ss_alb_wtr_avg_cld(bnd_idx) = 0.001/0.963
                           endif 
         ...
         !H. Wang
                  ...
                  mu0 = mu_not
                  do i = snl_top,nbr_lyr,1
                     ! write(*,*)'i',i,'bnd_idx',bnd_idx,'flg_slr_in',flg_slr_in,'trntdr',trntdr(i)
                  ...
                  ! of radiation to the interface just above the layer exceeds trmin.
                     ...
                     if (trntdr(i) > trmin ) then
                        ! write(*,*)i,'trntdr(i) > trmin',flg_slr_in,bnd_idx
                        ...
                        if( i == kfrsnl1 .or. i == kfrsnl2 .or. i == kfrsnl3.or. i == kfrsnl4) then
                           ! write(*,*)'mu0n before',mu0n
                           ! write(*,*)'i',i,'bnd_idx',bnd_idx,'flg_slr_in',flg_slr_in,'nreal',nreal
                           ! write(*,*)'in mu0n kfrsnl'
                           mu0n      = max(cos(asin(sin(acos(mu_not))/nreal)),cp01)
                           ! write(*,*)'mu0n after',mu0n
                        endif
                        ts = tau_star(i)
                        ws = omega_star(i)
                        gs = g_star(i)
                  ...
                  do i = snl_top, nl_lake 
                     ...
                     write(*,*)i,'flx_abs_lcl',flx_abs_lcl(i,bnd_idx),bnd_idx
            ...
            if ( coszen > 0.) then 
               ...
               if((dirdn_rate(snl_top,1)*forc_sols+dirdn_rate(snl_top,2)*forc_soll   &
                                          +difdn_rate(snl_top,1)*forc_solsd+difdn_rate(snl_top,2)*forc_solld)/=0.) then
                  lakealb_direct_shortwave = (dirup_rate(snl_top,1)*forc_sols+difup_rate(snl_top,1)*forc_solsd&
                                          +dirup_rate(snl_top,2)*forc_soll+difup_rate(snl_top,2)*forc_solld)/&
