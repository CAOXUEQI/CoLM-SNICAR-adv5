...
module MOD_Vars_1DAccFluxes
   ...
   real(r8), allocatable :: a_up_dir_per   (:)
   real(r8), allocatable :: a_up_dif_per   (:)
   real(r8), allocatable :: a_phi_srf_dif   (:)
   real(r8), allocatable :: a_phi_srf_dir   (:)
...
!end plant hydraulic parameters
   real(r8), allocatable :: a_phi_dir_per      (:,:)
   real(r8), allocatable :: a_phi_dif_per      (:,:)
   ...
   subroutine allocate_acc_fluxes
      ...
      if (p_is_worker) then
         if (numpatch > 0) then
            ...
            allocate (a_up_dir_per     (numpatch))
            allocate (a_up_dif_per     (numpatch))
            allocate (a_phi_srf_dif     (numpatch))
            allocate (a_phi_srf_dir     (numpatch))
...
!End Plant Hydraulic parameters
            allocate (a_phi_dir_per      (maxsnl+1:nl_lake,numpatch))
            allocate (a_phi_dif_per      (maxsnl+1:nl_lake,numpatch))
   ...
   subroutine deallocate_acc_fluxes ()
      ...
      if (p_is_worker) then
         if (numpatch > 0) then
            ...
            deallocate (a_up_dir_per     )
            deallocate (a_up_dif_per     )
            deallocate (a_phi_srf_dif     )
            deallocate (a_phi_srf_dir     )
            deallocate (a_us     )
            deallocate (a_vs     )
            deallocate (a_t      )
            deallocate (a_q      )
...
!Plant Hydraulic parameters
            deallocate (a_vegwp       )
!end plant hydraulic parameters
            deallocate (a_phi_dir_per      )
            deallocate (a_phi_dif_per      )
            deallocate (a_t_lake      )
   ...
   SUBROUTINE accumulate_fluxes
      ...
      if (p_is_worker) then
         if (numpatch > 0) then
            ...
            call acc1d (up_dir_per  , a_up_dir_per  )
            call acc1d (up_dif_per  , a_up_dif_per  )
            call acc1d (phi_srf_dif  , a_phi_srf_dif  )
            call acc1d (phi_srf_dir  , a_phi_srf_dir  )
            ...
            call acc2d (phi_dir_per      , a_phi_dir_per       )
            call acc2d (phi_dif_per      , a_phi_dif_per       )
