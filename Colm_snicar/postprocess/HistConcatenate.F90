...
program hist_concatenate
   ...
   call hist_concatenate_var_2d (filehist, 'f_up_dir_per   ', timelen, compress, &
   'direct up energy from the surface layer','%')

   call hist_concatenate_var_2d (filehist, 'f_up_dif_per   ', timelen, compress, &
   'diffuse up energy from the surface layer','%') 
   ! write(*,*)"f_up_dif_per done"

   call hist_concatenate_var_2d (filehist, 'f_phi_srf_dif   ', timelen, compress, &
   'surface absorb energy','W/m2')
   ! write(*,*)"f_phi_srf_dif done"
   call hist_concatenate_var_2d (filehist, 'f_phi_srf_dir   ', timelen, compress, &
   'surface absorb energy','W/m2')

   call hist_concatenate_var_3d (filehist, 'f_phi_dir_per   ', timelen, 'snowlake', &
      nl_lake-maxsnl, compress, 'direct in phi','%')

   call hist_concatenate_var_3d (filehist, 'f_phi_dif_per   ', timelen, 'snowlake', &
      nl_lake-maxsnl, compress, 'diffuse in phi','%')
